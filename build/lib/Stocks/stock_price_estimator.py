...
from Stocks.estimation_result import EstimationResult
import numpy as np

class StockPriceEstimator(ABC):
    def __init__(self, stock_utility: StockUtility) -> None:
        self.stock_utility = stock_utility

    def estimate_from_historic(self, historical_period: Period, future_days: int) -> EstimationResult:
        pass
...
class LeastSqaureMethodEstimator(StockPriceEstimator):
    ...
    def estimate_from_historic(self, historical_period: Period, future_days: int) -> EstimationResult:
        historic_close_prices = self.stock_utility.get_historical_data(historical_period).Close
